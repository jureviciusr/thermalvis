...
bool calibratorData::assignFromXml(xmlParameters &xP) {
    ...
    BOOST_FOREACH(boost::property_tree::ptree::value_type &v,
                  xP.pt.get_child("launch")) { // Within tree (pt), finds launch, and loops all tags within it
                    ...
                    if (v.second.get_child("<xmlattr>.type").data().compare("calibrator")) {
                        if ((!v.second.get_child("<xmlattr>.type").data().compare("reconfigure_gui")) ||
                            (!v.second.get_child("<xmlattr>.type").data().compare("rqt_reconfigure"))) {
                    ...
                    BOOST_FOREACH(boost::property_tree::ptree::value_type &v2, v.second) { // Traverses the subtree...
                                    ...
                                    if (!v2.second.get_child("<xmlattr>.name").data().compare("debugMode"))
                                        debugMode = !v2.second.get_child("<xmlattr>.value").data().compare("true");
                                    if (!v2.second.get_child("<xmlattr>.name").data().compare("verboseMode"))
                                        verboseMode = !v2.second.get_child("<xmlattr>.value").data().compare("true");
                                    if (!v2.second.get_child("<xmlattr>.name").data().compare(
                                            "outputFolder"))
                                        outputFolder = v2.second.get_child("<xmlattr>.value").data();

                                    if (!v2.second.get_child("<xmlattr>.name").data().compare("autoAlpha"))
                                        autoAlpha = !v2.second.get_child("<xmlattr>.value").data().compare("true");
                                    if (!v2.second.get_child("<xmlattr>.name").data().compare("undistortImages"))
                                        undistortImages = !v2.second.get_child("<xmlattr>.value").data().compare(
                                                "true");

                                    if (!v2.second.get_child("<xmlattr>.name").data().compare(
                                            "video_stream"))
                                        video_stream = v2.second.get_child("<xmlattr>.value").data();

                                    if (!v2.second.get_child("<xmlattr>.name").data().compare("calibType"))
                                        calibType = v2.second.get_child("<xmlattr>.value").data();
                                    if (!v2.second.get_child("<xmlattr>.name").data().compare("patternType"))
                                        patternType = v2.second.get_child("<xmlattr>.value").data();
                                    if (!v2.second.get_child("<xmlattr>.name").data().compare("optMethod"))
                                        optMethod = v2.second.get_child("<xmlattr>.value").data();


                                    if (!v2.second.get_child("<xmlattr>.name").data().compare("maxPatterns"))
                                        maxPatterns = atoi(v2.second.get_child("<xmlattr>.value").data().c_str());
                                    if (!v2.second.get_child("<xmlattr>.name").data().compare("maxFrames"))
                                        maxFrames = atoi(v2.second.get_child("<xmlattr>.value").data().c_str());
                                    if (!v2.second.get_child("<xmlattr>.name").data().compare("setSize"))
                                        setSize = atoi(v2.second.get_child("<xmlattr>.value").data().c_str());
                                    if (!v2.second.get_child("<xmlattr>.name").data().compare("xCount"))
                                        xCount = atoi(v2.second.get_child("<xmlattr>.value").data().c_str());
                                    if (!v2.second.get_child("<xmlattr>.name").data().compare("yCount"))
                                        yCount = atoi(v2.second.get_child("<xmlattr>.value").data().c_str());

                                    if (!v2.second.get_child("<xmlattr>.name").data().compare("gridSize"))
                                        gridSize = atof(v2.second.get_child("<xmlattr>.value").data().c_str());
                                    if (!v2.second.get_child("<xmlattr>.name").data().compare("alpha"))
                                        alpha = atof(v2.second.get_child("<xmlattr>.value").data().c_str());
...
void calibratorNode::prepareExtrinsicPatternSubsets() {
    ...
    while ((((int) extrinsicCandidateSets[0].size()) < configData.maxCandidates) && (configData.maxCandidates > 0) &&
           (tmpSet[0].size() > 0)) {
    ...
    while ((((int) extrinsicTestingSets[0].size()) < configData.maxTests) && (configData.maxTests > 0) &&
           (tmpSet[0].size() > 0)) {
...
void calibratorNode::create_virtual_pairs() {
    ...
    ROS_INFO("Extracted patterns for camera (1) : (%d/%d) & (2) : (%d/%d)", int(pointSets[0].size()),
             int(frameCount[0]), int(pointSets[1].size()), int(frameCount[1]));
    ...
    for (int aaa = 0; aaa < int(configData.numCams); aaa++) {
        for (int bbb = 0; bbb < int(configData.numCams); bbb++) {
            ...
            if (configData.verboseMode) {
                ROS_INFO("Finding virtual frames for camera relationship (%d) -> (%d).", aaa, bbb);
            }
            ...
            for (int iii = 0; iii < patternTimestamps[aaa].size(); iii++) {
                ...
                if (configData.verboseMode) {
                    ROS_INFO("Best index matches for (%d/%d) = (%d-%d/%d)", iii, int(patternTimestamps[aaa].size()),
                             matchingIndex - 1, matchingIndex, int(patternTimestamps[bbb].size()));
                }
                if (configData.verboseMode) {
                    ROS_INFO("Best index matches for (%d)[%f] = (%d)[%f] and (%d)[%f]", iii,
                             patternTimestamps[aaa].at(iii).toSec(), matchingIndex - 1,
                             patternTimestamps[bbb].at(matchingIndex - 1).toSec(), matchingIndex,
                             patternTimestamps[bbb].at(matchingIndex).toSec());
                }

                double interp_distance_1 = abs(
                        patternTimestamps[aaa].at(iii).toSec() - patternTimestamps[bbb].at(matchingIndex - 1).toSec());
                double interp_distance_2 = abs(
                        patternTimestamps[aaa].at(iii).toSec() - patternTimestamps[bbb].at(matchingIndex).toSec());
                double interp_distance = max(interp_distance_1, interp_distance_2);

                if (configData.verboseMode) {
                    ROS_INFO("Interpolation distance = (%f) {max(%f, %f)}", interp_distance, interp_distance_1,
                             interp_distance_2);
                }
                ...
                if (interp_distance < configData.maxInterpolationTimegap) {
                    ...
                    double biasFraction = (patternTimestamps[aaa].at(iii).toSec() -
                                           patternTimestamps[bbb].at(matchingIndex - 1).toSec()) /
                                          (patternTimestamps[bbb].at(matchingIndex).toSec() -
                                           patternTimestamps[bbb].at(matchingIndex - 1).toSec());

                    double motion = generateVirtualPointset(pointSets[bbb].at(matchingIndex - 1),
                                                            pointSets[bbb].at(matchingIndex), virtualPointset,
                                                            biasFraction);
    ...
    if (configData.verboseMode) {
        ROS_INFO("Found (%d // %d) virtual pairs", int(extrinsicsPointSets[0].size()),
                 int(extrinsicsPointSets[1].size()));
    }
...
void calibratorNode::preparePatternSubsets() {
    ...
    for (unsigned int xxx = 0; xxx < configData.numCams; xxx++) {
        ...
        // While the candidate quantity is insufficient
        while ((((double) candidateSets[xxx].size()) < actualMaxCandidates) && (configData.maxCandidates > 0) &&
               (tmpSet.size() > 0)) {
        ...
        // While the testing quantity is insufficient
        while ((((double) testingSets[xxx].size()) < actualMaxTests) && (configData.maxTests > 0) &&
               (tmpSet.size() > 0)) {
...
void calibratorNode::writeResults() {
    ...
    if (wantsIntrinsics()) {
        for (unsigned int xxx = 0; xxx < configData.numCams; xxx++) {
            ...
            imageSize.at < unsigned
            short > (0, 0) = imSize[xxx].width;
            imageSize.at < unsigned
            short > (0, 1) = imSize[xxx].height;
    ...
    if (wantsExtrinsics()) {
        ...
        if (!configData.fixIntrinsics) {
            ...
            for (unsigned int xxx = 0; xxx < configData.numCams; xxx++) {
                ...
                newCamMat[xxx] = getOptimalNewCameraMatrix(cameraMatrices[xxx], distCoeffVecs[xxx], imSize[xxx],
                                                           configData.alpha, imSize[xxx], validPixROI,
    ...
    */
    ...
    for (int i = 0; i < 2; i++) {
        ...
        if ((pt1.x == 0) && (pt1.y == 0) && (pt2.x == 0) && (pt2.y == 0)) {
            ...
            rectCamMat[i] = getOptimalNewCameraMatrix(cameraMatrices[i], distCoeffVecs[i], imSize[i], configData.alpha,
                                                      imSize[i], &validROI_x);
...
*/
    ...
    for (int k = 1; k < 32; k++) {
        ...
        rightLinePoints.push_back(
                Point2f(float(imSize[0].width), float(k * (botValidHeight - topValidHeight) / 32 - 1)));
    }
...
//#endif
    ...
    //HGH - draw roi
    rectangle(disp1, Point2f(float(roi[0].x), float(roi[0].y)),
              Point2f(float(roi[0].x + roi[0].width), float(roi[0].y + roi[0].height)), CV_RGB(127, 255, 0));
    ...
    //HGH - draw roi
    rectangle(disp2, Point2f(float(roi[1].x), float(roi[1].y)),
              Point2f(float(roi[1].x + roi[1].width), float(roi[1].y + roi[1].height)), CV_RGB(127, 255, 0));
...
void calibratorNode::performExtrinsicCalibration() {
    ...
    ROS_INFO("Optimizing calibration sets, (%d) candidates and (%d) testing patterns",
             int(extrinsicCandidateSets[0].size()), int(extrinsicTestingSets[0].size()));
    ...
    if (configData.fixIntrinsics) {
        ...
        extrinsicsFlags += CALIB_FIX_INTRINSIC + CALIB_FIX_FOCAL_LENGTH + CALIB_FIX_K6 + CALIB_FIX_K5 + CALIB_FIX_K4 +
                           CALIB_FIX_K3 + CALIB_FIX_K2 + CALIB_FIX_K1;
    } else {
    ...
    ROS_WARN("Optimizing with (%d) candidates and (%d) test patterns...", int(extrinsicCandidateSets[0].size()),
             int(extrinsicTestingSets[0].size()));

    optimizeCalibrationSets(extrinsicsSizes, 2, cameraMatrices, distCoeffVecs, extrinsicsDistributionMap,
                            extrinsicCandidateSets, extrinsicTestingSets, row, configData.optimizationMethod,
                            configData.setSize, extrinsicTagNames, extrinsicSelectedTags, extrinsicsFlags);
    ...
    for (unsigned int iii = 0; iii < extrinsicCandidateSets[0].size(); iii++) {
        for (int j = 0; j < configData.yCount; j++) {
            for (int i = 0; i < configData.xCount; i++) {
                objectPoints[iii].push_back(Point3f(float(j) * float(configData.gridSize / 1000.0),
                                                    float(i) * float(configData.gridSize / 1000.0), 0));
            }
        }
    }
...
//*/
    ...
    extendedExtrinsicReprojectionError = calculateExtrinsicERE(2, objectPoints.at(0), extrinsicsPointSets,
                                                               cameraMatrices, distCoeffVecs, R, T);
...
void calibratorNode::performIntrinsicCalibration() {
    ...
    if (configData.ignoreDistortion)
        intrinsicsFlags += CALIB_FIX_K6 + CALIB_FIX_K5 + CALIB_FIX_K4 + CALIB_FIX_K3 + CALIB_FIX_K2 + CALIB_FIX_K1 +
    ...
    for (int xxx = 0; xxx < int(configData.numCams); xxx++) {
        ...
        ROS_INFO("Intrinsically calibrating camera (%d) with (%d / %d / %d) patterns...", xxx,
                 int(candidateSets[xxx].size()), int(testingSets[xxx].size()), int(pointSets[xxx].size()));

        bool debugOptimizationFunction = false;
        optimizeCalibrationSet(imSize[xxx], candidateSets[xxx], testingSets[xxx], row, subselectedTags_intrinsics[xxx],
                               configData.optimizationMethod, configData.setSize, debugOptimizationFunction,
                               configData.removeSpatialBias, configData.generateFigures,
                               configData.useUndistortedLocations, intrinsicsFlags);
        ...
        reprojectionError_intrinsics[xxx] = calibrateCamera(objectPoints, subselectedPatterns, imSize[xxx],
                                                            cameraMatrices[xxx], distCoeffVecs[xxx], rvecs, tvecs,
                                                            intrinsicsFlags);

        ROS_INFO("Reprojection error = %f, distCoeffVecs[%d].size() = (%d)", reprojectionError_intrinsics[xxx], xxx,
                 distCoeffVecs[xxx].cols);
        ...
        extendedReprojectionError_intrinsics[xxx] = calculateERE(imSize[xxx], objectPoints.at(0), testingSets[xxx],
                                                                 cameraMatrices[xxx], distCoeffVecs[xxx],
                                                                 configData.removeSpatialBias,
                                                                 configData.generateFigures,
                                                                 configData.useUndistortedLocations);

        ROS_INFO("Extended Reprojection error = %f (%d patterns)", extendedReprojectionError_intrinsics[xxx],
                 int(testingSets[xxx].size()));
    }
...
bool calibratorData::processStartingData() {
    ...
    if (calibType == "single") {
        if (wantsIntrinsics == false) {
            ROS_WARN(
                    "<single> camera mode has been selected, but <wantsIntrinsics> has not. Intrinsics WILL be calculated.");
        ...
        if (wantsIntrinsics) { ROS_INFO("<stereo> mode: option to also calculate intrinsics has been selected."); }
            ...
            if (intrinsicsFiles[0] == "intrinsicsFile")
                ROS_WARN(
                        "No intrinsics specified for primary camera, so will attempt to use those encoded in topic header.");
            if (intrinsicsFiles[1] == "intrinsicsFile")
                ROS_WARN(
                        "No intrinsics specified for secondary camera, so will attempt to use those encoded in topic header.");
        }
    }

    if (verboseMode) {
        if (debugMode) { ROS_INFO("Running in DEBUG mode."); } else {
            ROS_INFO("Running in OPTIMIZED mode.");
        }
    }
...
void calibratorNode::updateIntrinsicMap(unsigned int idx) {
    ...
    newCamMat[idx] = getOptimalNewCameraMatrix(cameraMatrices[idx], distCoeffVecs[idx], imSize[idx], configData.alpha,
                                               imSize[idx], validPixROI, centerPrincipalPoint);

    if (configData.verboseMode) { ROS_WARN("Adjusting undistortion mapping..."); }
    initUndistortRectifyMap(cameraMatrices[idx], distCoeffVecs[idx], default_R, newCamMat[idx], imSize[idx], CV_32FC1,
                            map1[idx], map2[idx]);
...
bool calibratorNode::findPattern(const Mat &im, vector<Point2f> &dst, Mat &prev, const int cameraNumber = -1) {
    ...
    if ((configData.trackingMode) && (dst.size() > 0) && (prev.rows > 0) && (prev.cols > 0)) {
        ...
        int distanceConstraint = int((min(im.cols, im.rows) * configData.maxFrac) +
                                     ((((int) (max(im.cols, im.rows) * configData.maxFrac)) + 1) % 2));
        ...
        trackPoints2(prev, im, src, dst, distanceConstraint, configData.flowThreshold, lost,
                     cvSize(configData.xCount, configData.yCount), configData.errorThreshold);
        ...
        cornerSubPix(im, dst, Size(windowSize, windowSize), Size(-1, -1),
        ...
        switch (configData.pattType) {
            ...
            case MASK_FINDER_CODE:
                if (configData.adjustMSER[0] && cameraNumber == 0) {
                    retVal = findMaskCorners(im, cvSize(configData.xCount, configData.yCount), dst,
                                             PATTERN_FINDER_CV_CORNER_SUBPIX_FLAG, configData.delta[0],
                                             configData.maxVar[0], configData.minDiv[0]);
                } else if (configData.adjustMSER[1] && cameraNumber == 1) {
                    retVal = findMaskCorners(im, cvSize(configData.xCount, configData.yCount), dst,
                                             PATTERN_FINDER_CV_CORNER_SUBPIX_FLAG, configData.delta[1],
                                             configData.maxVar[1], configData.minDiv[1]);
                } else {
                    retVal = findMaskCorners(im, cvSize(configData.xCount, configData.yCount), dst,
                                             PATTERN_FINDER_CV_CORNER_SUBPIX_FLAG);
                }
                break;
    ...
    if (configData.debugMode) {
        ...
        for (unsigned int i = 0; i < validPatternData[0].size(); i++) {
            ...
            if (validPatternData[0].at(i).patternFound && validPatternData[1].at(i).patternFound) {
                ...
                ROS_INFO("\nValid pattern pair %d out of  %d :  frame cam1: %d, frame cam2 %d ",
                         currentValidPattern + 1, totalValidPatterns, validPatternData[0].at(i).patternID,
                         validPatternData[1].at(i).patternID);
                ...
                drawChessboardCorners(dispMat1, cvSize(configData.xCount, configData.yCount),
                                      Mat(pointSets[0].at(dispPattern1)), true);
                ...
                drawChessboardCorners(dispMat2, cvSize(configData.xCount, configData.yCount),
                                      Mat(pointSets[1].at(dispPattern2)), true);
...
//preprocess images
void calibratorNode::preprocessImage(Mat src, Mat &dst, double a = 1.0, double b = 0.0, bool normalize = false,
                                     bool negative = false) {
...
void calibratorNode::updatePairs() {
    ...
    if (validPairs[0].size() > publishCount) {
        ...
        if ((validPairs[0].at(currentPair) >= displayImages[0].size()) ||
            (validPairs[1].at(currentPair) >= displayImages[1].size())) {
            ROS_WARN("Skipping detection because there appear to be insufficient buffered frames [(%d)(%d) : (%d)(%d)",
                     validPairs[0].at(currentPair), int(displayImages[0].size()), validPairs[1].at(currentPair),
                     int(displayImages[1].size()));
            return;
        }
        ...
        switch (configData.pattType) {
            case CHESSBOARD_FINDER_CODE:
                patternFound_1 = findChessboardCorners(grayMat1, cvSize(configData.xCount, configData.yCount),
                                                       cornerSet_1);
                patternFound_2 = findChessboardCorners(grayMat2, cvSize(configData.xCount, configData.yCount),
                                                       cornerSet_2);
                break;
            case MASK_FINDER_CODE:
                patternFound_1 = findMaskCorners(grayMat1, cvSize(configData.xCount, configData.yCount), cornerSet_1,
                                                 PATTERN_FINDER_CV_CORNER_SUBPIX_FLAG);
                patternFound_2 = findMaskCorners(grayMat2, cvSize(configData.xCount, configData.yCount), cornerSet_2,
                                                 PATTERN_FINDER_CV_CORNER_SUBPIX_FLAG);
                break;
            case HEATED_CHESSBOARD_FINDER_CODE:
                invertMatIntensities(grayMat1, tmpMat1);
                patternFound_1 = findChessboardCorners(tmpMat1, cvSize(configData.xCount, configData.yCount),
                                                       cornerSet_1);

                invertMatIntensities(grayMat2, tmpMat2);
                patternFound_2 = findChessboardCorners(tmpMat2, cvSize(configData.xCount, configData.yCount),
                                                       cornerSet_2);
        ...
        if (patternFound_1 && patternFound_2) {
            ...
            ROS_INFO("(%d) patterns found from (%d) valid frame pairs...", int(extrinsicsPointSets[0].size()),
                     int(validPairs[0].size()));
        ...
        if (configData.debugMode) {
            ...
            drawChessboardCorners(dispMat1, cvSize(configData.xCount, configData.yCount), Mat(cornerSet_1),
                                  patternFound_1);
            ...
            drawChessboardCorners(dispMat2, cvSize(configData.xCount, configData.yCount), Mat(cornerSet_2),
                                  patternFound_2);
...
void calibratorNode::handle_camera(const cv::Mat &inputImage, const sensor_msgs::CameraInfo *info_msg,
    ...
    if ((configData.stopCapturingAtLimit) ||
        ((((int) totalMinPatterns) > configData.maxPatterns) && (configData.maxPatterns > 0)) ||
        ((((int) frameCount[camera_index]) > configData.maxFrames) && (configData.maxFrames > 0))) {
    ...
    while (!infoProcessed[camera_index]) {
        ...
        if (configData.wantsIntrinsics) {
            ...
            if (configData.intrinsicsFiles[camera_index] != "intrinsicsFile") {
                ...
                ROS_WARN("For camera (%d), using intrinsics file (%s)", camera_index,
                         configData.intrinsicsFiles[camera_index].c_str());

                if (configData.intrinsicsFiles[camera_index].at(0) != '/') {
                    configData.intrinsicsFiles[camera_index] =
                            configData.read_addr + configData.intrinsicsFiles[camera_index];
                }
                ...
                try {
                    ...
                    ROS_ERROR("Some failure in reading in the camera parameters for camera (%d) from a file.",
                              camera_index);
                }
                ...
                try {
                    ...
                    ROS_ERROR("Some failure in reading in the camera parameters for camera (%d) from the topic header.",
                              camera_index);
                }
    ...
    if (infoProcessed[camera_index]) {
        ...
        if (duplicateFlags[camera_index].at(duplicateFlags[camera_index].size() - 1) == 0) {
            ...
            if (configData.debugMode) {
                ...
                drawChessboardCorners(dispMat, cvSize(configData.xCount, configData.yCount),
                                      Mat(cornerSet[camera_index]), patternFound);
...
calibratorNode::calibratorNode(calibratorData startupData) :
    ...
    for (int iii = 0; iii < configData.yCount; iii++) {
        for (int jjj = 0; jjj < configData.xCount; jjj++) {
            row.push_back(Point3f(((float) iii) * float(configData.gridSize / 1000.0),
                                  ((float) jjj) * float(configData.gridSize / 1000.0), 0.0));
        }
    }
...
void calibratorNode::loopCallback() {
    ...
    if ((elapsedInputTime / 1000.0 > configData.maxTime) && (configData.maxTime > 0)) {
        ROS_WARN("Maximum data collection time of (%f) seconds reached, terminating collection procedure..",
                 configData.maxTime);
        stillCollecting = false;
    }
